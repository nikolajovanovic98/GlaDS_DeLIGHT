...
if __name__=='__main__':
	...
	print(f"Mesh directory: {mesh}")
	print(f"Moulin file: {moulin_file}")
	print(f"Number of partitions: {nbpartition}")
	...
	if nbpartition >1:
		mesh_dir = '%s/partitioning.%s'%(mesh,np.str_(nbpartition))
		print(mesh_dir)
	...
	for kk in np.arange(nbpartition): 
		if nbpartition >1:
			nodes_file='%s/part.%s.nodes'%(mesh_dir,np.str_(kk+1))
			elements_file='%s/part.%s.elements'%(mesh_dir,np.str_(kk+1))
			bc_file='%s/part.%s.boundary'%(mesh_dir,np.str_(kk+1))
			header_file='%s/part.%s.header'%(mesh_dir,np.str_(kk+1))
		else:
